...
class Detect(nn.Module):
    ...
    onnx_export_simplify=False  #ONNX export simplify for gap8
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        a = torch.tensor(anchors).float().view(self.nl, -1, 2) #anchor reshaped as (nl=2,na=3,2), 2 head,3anchors,2 width/height of anchors
    ...
    def forward(self, x):
        if self.onnx_export_simplify:#simplify onnx export without postproc
            z = []  # inference output
        ...
        else: #normal training/inference at pytorch
...
class Model(nn.Module):
    def __init__(self, cfg='yolov3.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names, range(nc)
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers. here m.f can be [-1,8]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings, this eval(m) convert string m to <class >
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR]:
            c1, c2 = ch[f], args[0] #c1 in_channel, c2 out_channel
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]] #add c1 in_channel, got full args for Conv
        ...
        m_ = nn.Sequential(*[m(*args) for _ in range(n)]) if n > 1 else m(*args)  # module, repeat n times
        t = str(m)[8:-2].replace('__main__.', '')  # module type, str(m)[0:8]=<class '
