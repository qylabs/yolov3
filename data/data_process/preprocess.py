...
if __name__=="__main__":
    ...
    # target_root='/home/xinglong/project/yolov3/data/WiderPerson_gray'
    data_root='/home/wangtao/project/data/WiderPerson_gray'
    target_root='/home/wangtao/project/yolov3/data/WiderPerson_gray'


    ##Build train images
    img_txt=data_root+'/train.txt'
    img_path=data_root+'/Images'
    target_path=target_root+'/images/train'
    ImgFolder(img_txt,img_path,target_path)

    # ###Build val images
    img_txt=data_root+'/val.txt'
    img_path=data_root+'/Images'
    target_path=target_root+'/images/val'
    ImgFolder(img_txt,img_path,target_path)

    # ###Build train labels
    img_txt=data_root+'/train.txt'
    label_path=data_root+'/Annotations'
    target_path=target_root+'/labels/train'
    LabelFolder(img_txt,label_path,target_path)
    
    ###Build val labels
    img_txt=data_root+'/val.txt'
    label_path=data_root+'/Annotations'
    target_path=target_root+'/labels/val'
    LabelFolder(img_txt,label_path,target_path)

    ####Update label format to yolo format
    label_path=target_root+'/labels/train'
    formatLabels(label_path)


    label_path=target_root+'/labels/val'
    formatLabels(label_path)
