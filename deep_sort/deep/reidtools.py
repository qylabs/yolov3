...
def visualize_ranked_results(
    distmat, dataset, data_type, width=64, height=128, save_dir='', topk=10
):
    ...
    for q_idx in range(num_q):
        ...
        for g_idx in indices[q_idx, :]:
            gimg_path, g_id, = gallery[g_idx]
            dist=distmat[q_idx][g_idx]
            ...
            if data_type == 'image':
                ...
                gimg = cv2.resize(gimg, (width, height))
                cv2.putText(gimg, str(dist), (5, height-5), cv2.FONT_HERSHEY_SIMPLEX, 1/2, [0, 0, 255], thickness=2)
