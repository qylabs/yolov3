...
from models.yolo import Model as Model2
...
def train(hyp, opt, device, tb_writer=None):
    ...
    model2 = Model2(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                pred = model(imgs)  # forward #yolo detect pred [x(bs,3,20,20,85)], len(pred)=nl
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                for k in model2.state_dict().keys():
                    model2.state_dict()[k].copy_(model.state_dict()[k])
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                orig_model = deepcopy(de_parallel(model2))
                ...
                if wandb_logger.wandb:
                    if ((epoch + 1) % opt.save_period == 0 and not final_epoch) and opt.save_period != -1:
                        wandb_logger.log_model(
                            qlast.parent, opt, epoch, fi, best_model=best_fitness == fi)
                del ckpt
            # Save model
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
