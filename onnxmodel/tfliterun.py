...
def yolo_proc(img_path,onnx_path,conf_thres=0.2,iou_thres=0.45):
    ...
    out=tfliterun(onnx_path,input_array)
...
def yolo_proc_simp(img_path,onnx_path,strides,anchors,nc,conf_thres=0.2,iou_thres=0.45):
    ...
    print(input_array.shape)
    out=tfliterun(onnx_path,input_array)
    print('tflite out.shape ',len(out))
...
def yolo_proc_simp_1H(img_path,onnx_path,strides,anchors,nc,conf_thres=0.2,iou_thres=0.45):
    ...
    out1=tfliterun(onnx_path,input_array)
    np.set_printoptions(threshold=np.inf)
    print('tflite out1.shape ',len(out1),out1[0])
...
def yolo_proc_simp_2H_noreshape_nosigmoid(img_path,onnx_path,strides,anchors,nc,conf_thres=0.2,iou_thres=0.45,iter=False):
    ...
    out=tfliterun(onnx_path,input_array)
    print('==>> 2H output tflite out.shape ',len(out))
...
if __name__=="__main__":
    ...
    # compute tflite Runtime output prediction
    batch_size=1
    channels=1#3
    height=128 #320
    width=160 #320
    ...
    print("==============================")
    tflite_path='/home/allen/work/quant2/onnx_tflite_yolov3/tmp/yolo_models/qat_quant_fullint.tflite'
    yolo_proc_simp_2H_noreshape_nosigmoid(img_path,tflite_path,strides,anchors,nc=1,conf_thres=0.2,iou_thres=0.45,iter=True)
